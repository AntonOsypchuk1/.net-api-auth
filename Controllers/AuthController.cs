...
namespace DotnetAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult Register(UserForRegistrationDto userForRegistrationDto)
        {
            if (userForRegistrationDto.Password == userForRegistrationDto.PasswordConfirm)
            {
                string sqlCheckUserExists =
                    $"SELECT Email FROM TutorialAppSchema.Auth WHERE Email = '{userForRegistrationDto.Email}'";

                IEnumerable<string> existingUsers = _dapper.LoadData<string>(sqlCheckUserExists);
                if (!existingUsers.Any())
                {
                    UserForLoginDto userForSetPassword =
                        new()
                        {
                    ...
                    {
                        UserComplete userComplete = _mapper.Map<UserComplete>(
        ...
        public IActionResult Login(UserForLoginDto userForLogin)
        {
            ...
            UserForLoginConfirmationDto userForLoginConfirmationDto =
            ...
            byte[] passwordHash = _authHelper.GetPasswordHash(
                ...
                userForLoginConfirmationDto.PasswordSalt
            );

            // if (passwordHash == userForLoginConfirmationDto.PasswordHash) // Won't work
